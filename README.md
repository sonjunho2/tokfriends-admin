...
- `EXPO_PUBLIC_ADMIN_OVERRIDE_CODES`: 관리자 비상 인증번호 목록입니다. 관리자 패널의 **설정 > 관리자 인증번호** 카드에서 값을 확인·수정하면 `admin-web/.env.local` 파일이 자동으로 갱신됩니다. 모바일 앱과 동일한 인증번호를 사용하려면 두 레포지터리의 이 값을 반드시 동일하게 유지하세요.
