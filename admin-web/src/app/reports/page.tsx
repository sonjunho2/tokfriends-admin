...
export default function ReportsPage() {
  ...
  // ✅ 빈 문자열 금지. 기본값은 'ALL' 로 두고, 요청 시엔 제외.
  ...
  const handleStatusChange = async (reportId: string, status: 'PENDING' | 'RESOLVED' | 'REJECTED') => {
    try {
        ...
        <div className="flex gap-2">
          {report.status === 'PENDING' && (
            <>
              ...
              <Button size="sm" variant="destructive" onClick={() => handleStatusChange(report.id, 'REJECTED')}>
                반려
              </Button>
            </>
          )}
  ...
  return (
    <div>
      ...
      <DataTable columns={columns as any} data={reports} loading={loading} />
